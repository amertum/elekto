...
import static elekto.results.model.College.Denomination.UNIQUE;
...
public class OperationLoader {
            ...
            final HSSFSheet etablissements)
    {
        ...
        for (int i = 2; (i <= lastRowIndex); i++) {
            ...
            if (electionInitialized && this.isRowNewElection(electionBuilder, rowValues, etablissementsSheet)) {
    ...
    {
        final Election rowElection = new RowElectionBuilder(row, etablissementsSheet).build(new ElectionBuilder()).toElection();
        final Election currentElection = electionBuilder.toElection();

        final boolean newElection = !currentElection.equals(rowElection);
        this.logger.debug("isRowNewElection=" + newElection + " => currentElection : " + currentElection
                + "; rowElection : " + rowElection);
        return newElection;
    }
            ...
            final CandidatRowValueGetter row)
    {
        ...
        this.logger.debug("isRowNewListe=" + newListe + " => currentListe : " + currentListe + "; rowListe : "
                + rowListe);
        return newListe;
    }
            ...
            final CandidatRowValueGetter row)
    {
        ...
        this.logger.debug("isRowNewCandidat=" + true + " => currentCandidat : " + currentCandidat + "; rowCandidat : "
                + rowCandidat);
        return true;
    }
            ...
            final HSSFRow row)
    {
        if (this.logger.isDebugEnabled()) {
            ...
            this.logger.debug(rowStr + " (index:" + row.getRowNum() + ")");
        }
    }
    ...
    static class RowElectionBuilder {
                ...
                final ElectionBuilder builder)
        {
            ...
            // TODO college denomination
            builder.college(UNIQUE, transform(this.row.getColleges(), new College.CollegeEnumFunction()));
